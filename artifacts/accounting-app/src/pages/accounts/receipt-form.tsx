...
import { ArrowLeft, ListChecks, CheckSquare, Square, Plus, X, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function ReceiptForm() {
  ...
  // Primary ledger amount (for the Receipt Ledger selected above)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!totalMatches) {
      toast({ title: "Amount mismatch", description: "Ledger allocation total must equal the receipt amount.", variant: "destructive" });
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
      ...
      <Card>
        <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <Select value={form.ledgerId} onValueChange={v => { set("ledgerId", v); setPrimaryAmount(""); setExtraRows([]); }}>
          ...
          {form.ledgerId && (
            <div className="sm:col-span-2 space-y-2">
              ...
              {/* Primary row (locked to selected Receipt Ledger) */}
      ...
      {hasAllocation && !totalMatches && (
        <p className="text-xs text-red-600">Ledger allocation total ({formatCurrency(totalAllocated)}) must equal receipt amount ({formatCurrency(totalRequired)}) to save.</p>
      )}
