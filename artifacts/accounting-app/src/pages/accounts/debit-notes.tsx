...
import { useListDebitNotes, useDeleteDebitNote, customFetch } from "@workspace/api-client-react";
...
function DebitNoteViewSheet({ id, onClose }: { id: number | null; onClose: () => void; }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setData(null);
    customFetch<any>(`/api/debit-notes/${id}`)
      .then(d => setData(d))
      .catch(() => setData(null))
          ...
          <SheetTitle className="flex items-center justify-between gap-2 pr-6">
            ...
            {id && (
              <div className="flex items-center gap-1.5">
                <Link href={`/accounts/debit-notes/${id}`}>
                  <Button size="sm" variant="outline" className="h-8 gap-1 text-xs">
                    <Printer className="h-3.5 w-3.5" /> Full Print
                  </Button>
                </Link>
                <Link href={`/accounts/debit-notes/${id}/edit`}>
                  <Button size="sm" variant="outline" className="h-8 gap-1 text-xs">
        ...
        {loading ? (
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="rounded-lg border bg-muted/30 divide-y">
              ...
              <div className="flex justify-between px-4 py-2.5">
                <span className="text-xs text-muted-foreground uppercase tracking-wide">Supplier / Party</span>
            ...
            <div className="rounded-lg border bg-muted/30 divide-y text-sm">
              ...
              <div className="flex justify-between px-4 py-2.5 font-bold text-base text-red-700">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Purchase returns — stock removed from inventory</p>
        </div>
        <Link href="/accounts/debit-notes/new"><Button size="sm"><Plus className="h-4 w-4 mr-1" />New</Button></Link>
      </div>
      ...
      <div className="md:hidden space-y-3">
        {isLoading ? (
          ...
          <div className="text-center text-muted-foreground py-10">No debit notes</div>
        ) : paginated.map((n: any) => (
          <Card key={n.id} className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => setViewId(n.id)}>
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex gap-2 border-t pt-3" onClick={e => e.stopPropagation()}>
                ...
                <Link href={`/accounts/debit-notes/${n.id}/edit`} className="flex-1">
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={n.id} className="cursor-pointer hover:bg-muted/40" onClick={() => setViewId(n.id)}>
                  ...
                  <TableCell className="text-right text-red-600 font-medium">{formatCurrency(n.amount)}</TableCell>
                  <TableCell onClick={e => e.stopPropagation()}>
                    <div className="flex gap-1">
                      ...
                      <Link href={`/accounts/debit-notes/${n.id}/edit`}><Button size="icon" variant="ghost" className="h-7 w-7" title="Edit"><Pencil className="h-3.5 w-3.5" /></Button></Link>
      ...
      <DebitNoteViewSheet id={viewId} onClose={() => setViewId(null)} />
    </div>
  );
}
