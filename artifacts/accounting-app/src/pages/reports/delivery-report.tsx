...
export default function DeliveryReport() {
  ...
  const { sortedData, sortKey, sortDir, setSortKey, setSortDir, toggleSort } = useReportSort(orders, "orderNumber", "asc");
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <ReportActions
            ...
            filename={`delivery-report-${from}-${to}`}
            title="Delivery Report"
            shareSummary={`Orders Dispatched: ${(data as any)?.totalOrders || 0}, Total: ${formatCurrency((data as any)?.totalAmount || 0)}`}
          />
        </div>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {vis.has("orderNumber") && <TableHead className="cursor-pointer select-none" onClick={() => toggleSort("orderNumber")}>Order# {sortKey === "orderNumber" ? (sortDir === "asc" ? "↑" : "↓") : ""}</TableHead>}
                ...
                {vis.has("grandTotal") && <TableHead className="text-right cursor-pointer select-none" onClick={() => toggleSort("grandTotal")}>Amount {sortKey === "grandTotal" ? (sortDir === "asc" ? "↑" : "↓") : ""}</TableHead>}
