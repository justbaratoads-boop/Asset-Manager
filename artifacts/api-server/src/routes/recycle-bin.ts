...
router.get("/recycle-bin", authMiddleware, async (_req, res) => {
  ...
  const items = [
    ...
    ...creditNotes.map(r => ({ id: r.id, type: "credit_note", title: r.noteNumber, subtitle: r.partyName, amount: Number(r.amount), date: r.date, deletedAt: r.updatedAt.toISOString(), expiresAt: expiresAt(r.updatedAt), daysLeft: daysLeft(r.updatedAt) })),
    ...debitNotes.map(r => ({ id: r.id, type: "debit_note", title: r.noteNumber, subtitle: r.partyName, amount: Number(r.amount), date: r.date, deletedAt: r.updatedAt.toISOString(), expiresAt: expiresAt(r.updatedAt), daysLeft: daysLeft(r.updatedAt) })),
