...
async function run() {
  try {
    ...
    for (const row of res.rows) {
      ...
      const itemsRes = await pool.query(`SELECT id, name FROM ${schema}.stock_items WHERE name ILIKE '%honda%' OR name ILIKE '%activa%'`);
      if (itemsRes.rows.length > 0) {
        console.log('Found in schema:', schema, itemsRes.rows);
      }
    }
    console.log('Done searching honda/activa.');
  } catch (err) {
    console.error(err);
  } finally {
    process.exit(0);
